...
export class ChartComponent {
  ...
  getPrices(
    ...
    for (let i = 0; i < total; i++) {
      const _p = this.generateOHLCNumbers(minPrice, maxPrice, volotility);
      prices.push(_p);
    }
    return prices;
  }
  generateOHLCNumbers(minPrice: number, maxPrice: number, volatility: number) {
    ...
    const priceRange = maxPrice - minPrice; // Calculate the price range

    // Generate random opening price within the range
    const openPrice = Math.random() * priceRange + minPrice;

    // Calculate a random volatility factor between -volatility and +volatility
    const volatilityFactor = (Math.random() - 0.5) * 2 * volatility;

    // Calculate adjusted close price based on open price and volatility
    const closePrice = Math.max(
      minPrice,
      Math.min(maxPrice, openPrice * (1 + volatilityFactor))
    );

    // Ensure low price is lower than all other values
    const lowPrice = Math.min(
      openPrice,
      closePrice,
      Math.random() * (closePrice - minPrice) + minPrice
    );

    // Ensure high price is higher than all other values
    const highPrice = Math.max(
      openPrice,
      closePrice,
      Math.random() * (maxPrice - closePrice) + closePrice
    );
